...
class TitleState extends MusicBeatState
{
  ...
  function startIntro()
  {
    ...
    (new FlxTimer()).start(Conductor.stepCrochet / 1000 * 2, (_) -> {
      ...
      FlxTween.tween(blackSprite, {alpha: 0}, Conductor.stepCrochet / 1000 * 6,
        {
          onComplete: (_) -> {
            transitioning = false;
      ...
      FlxTween.tween(curtain, {y: -682.501606766917}, Conductor.stepCrochet / 1000 * 4,
        {ease: FlxEase.circOut, startDelay: (Conductor.stepCrochet / 1000) / 8});
      camGame.zoom += 0.075;
  ...
  override function update(elapsed:Float)
  {
    ...
    if (ntsc != null) ntsc.uFrame.value = [Conductor.songPosition];

    if (staticShader != null) staticShader.iTime.value = [Conductor.songPosition];
    ...
    if (pressedEnter && !transitioning)
    {
      ...
      enterSprite.animation.finishCallback = (_) -> {
  ...
  override function stepHit()
  {
    ...
    if (enterSprite != null && curStep % 8 == 0 && enterSprite.animation.name != "press") enterSprite.animation.play("idle", true);

    if (bf != null && curStep % 4 == 0) bf.animation.play("idle", true);

    if (gf != null && curStep % 4 == 0) gf.animation.play("idle", true);

    if (curStep % 4 == 0)
    {
      for (hand in hands)
      {
        if (hand != null) hand.animation.play("idle");
      }
    }
    ...
    if (curStep % 8 == 0)
    {
      for (shader in handShaders)
      {
        ...
        FlxTween.num(FlxG.random.float(1, 2), 0.5, (Conductor.stepCrochet / 1000) * 6,
          {
            onUpdate: (_:FlxTween) -> {
