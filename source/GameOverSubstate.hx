...
class GameOverSubstate extends MusicBeatSubstate
{
  ...
  public function new(x:Float, y:Float, camX:Float, camY:Float, state:PlayState)
  {
    ...
    if (PlayState.curStage != 'warioworld' && PlayState.curStage != 'racing' && PlayState.curStage != 'virtual' && PlayState.curStage != 'promoshow'
      && PlayState.curStage != 'wetworld' && PlayState.curStage != 'endstage' && PlayState.curStage != 'nesbeat' && PlayState.curStage != 'piracy'
      && PlayState.SONG.song != 'Oh God No' && PlayState.curStage != 'forest') camFollow = new FlxPoint(bf.getGraphicMidpoint().x, bf.getGraphicMidpoint().y);

    if (PlayState.curStage == 'meatworld') camFollow = new FlxPoint(bf.getGraphicMidpoint().x - 1000, bf.getGraphicMidpoint().y - 300);
    ...
    else if (PlayState.curStage == 'forest')
    {
      ...
      FlxTween.tween(cdGlitch3, {y: cdGlitch3.y + 150}, 1.3,
        {
          ease: FlxEase.quadInOut,
          onComplete: function(twn:FlxTween) {
            timers.push(new FlxTimer().start(2.1, function(tmr:FlxTimer) {
              FlxTween.tween(cdGlitch3, {y: cdGlitch3.y - 75}, 0.8,
                {
                  ease: FlxEase.quadInOut,
                  onComplete: function(twn:FlxTween) {
    ...
    else if (PlayState.curStage == 'piracy')
    {
      ...
      if (ClientPrefs.downScroll) pibehallyboo.y += 344;
      else
        pibehallytwo.y += 384;
  ...
  function acceptConfirm()
  {
    ...
    {
      if (PlayState.curStage == 'superbad') badMario.visible = false;
      if (characterName != 'bf-goomba') endBullshit();
      else if (bf.startedDeath) endBullshit();
    }
  }
  ...
  function doCustomGameOver()
  {
    switch (characterName)
    {
      case 'bfbaddeath':
        ...
        FlxTween.tween(badMario, {x: badMario.x + 30}, 0.15,
          {
            ease: FlxEase.quadIn,
            onComplete: function(twn:FlxTween) {
              if (!isEnding)
              ...
              FlxTween.tween(badMario, {y: badMario.y - 100}, 0.25,
                {
                  ease: FlxEase.quadOut,
                  onComplete: function(twn:FlxTween) {
                    FlxTween.tween(badMario, {y: badMario.y + 150}, 0.35,
                      {
                        ease: FlxEase.quadIn,
                        onComplete: function(twn:FlxTween) {
                          if (!isEnding)
                          ...
                          FlxTween.tween(badMario, {y: badMario.y - 40}, 0.15,
                            {
                              ease: FlxEase.quadOut,
                              onComplete: function(twn:FlxTween) {
                                FlxTween.tween(badMario, {y: badMario.y + 160}, 0.4,
                                  {
                                    ease: FlxEase.quadIn,
                                    onComplete: function(twn:FlxTween) {
                                      if (!isEnding)
                                      ...
                                      FlxTween.tween(badMario, {y: badMario.y - 40}, 0.15,
                                        {
                                          ease: FlxEase.quadOut,
                                          onComplete: function(twn:FlxTween) {
                                            FlxTween.tween(badMario, {y: badMario.y + 160}, 0.3,
                                              {
                                                ease: FlxEase.quadIn,
                                                onComplete: function(twn:FlxTween) {
                                                  badMario.animation.play('walk', true);
                                                  // walk away
                                                  FlxTween.tween(badMario, {x: badMario.x + 130}, 0.6,
                                                    {
                                                      ease: FlxEase.quadIn,
                                                      onComplete: function(twn:FlxTween) {
  ...
  function coolStartDeath(?volume:Float = 1):Void
  {
    if (PlayState.curStage != 'warioworld' && PlayState.curStage != 'racing' && PlayState.curStage != 'virtual')
    {
      ...
      timers.push(new FlxTimer().start(Math.floor(1 + (Conductor.bpm / 60)), function(tmr:FlxTimer) {
        if (!isEnding && bf.animOffsets.exists('deathLoop')) bf.playAnim('deathLoop', true);
  ...
  function playVoiceline(time:Float = 2.5):Void
  {
    if (hasVA)
    {
      ...
      timers.push(new FlxTimer().start(time, function(tmr:FlxTimer) {
        ...
        FlxTween.tween(FlxG.sound.music, {volume: startingVolume / 2}, 1,
          {
            onComplete: function(twn:FlxTween) {
  ...
  function endBullshit():Void
  {
    if (!isEnding)
    {
      ...
      {
        isEnding = true;
        if (bf.animOffsets.exists('deathConfirm')) bf.playAnim('deathConfirm', true);
        ...
        if (!songFadeOut) FlxG.sound.music.stop();
        else
          FlxTween.tween(FlxG.sound.music, {volume: 0}, 2.5);
        if (hasVA) voiceline.stop();
        FlxG.sound.play(Paths.music(endSoundName));
        if (PlayState.curStage == 'landstage') FlxG.sound.play(Paths.music('GBchuckle'));
