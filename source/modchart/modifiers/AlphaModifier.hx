...
class AlphaModifier extends NoteModifier
{
  ...
  public function getHiddenEnd(player:Int = -1)
  {
    return (FlxG.height / 2)
      ...
      + (FlxG.height / 2) * getSubmodValue("hiddenOffset", player);
  }
  ...
  override function updateNote(beat:Float, note:Note, pos:Vector3, player:Int)
  {
    ...
    var pos = modMgr.getPos(note.strumTime, modMgr.getVisPos(Conductor.songPosition, note.strumTime, PlayState.instance.songSpeed),
      note.strumTime - Conductor.songPosition, PlayState.instance.curBeat, note.noteData, player, note, ["reverse"]);

    var alphaMod = (1 - getSubmodValue("alpha",
      player)) * (1 - getSubmodValue('alpha${note.noteData}',
        player)) * (1 - getSubmodValue("noteAlpha", player)) * (1 - getSubmodValue('noteAlpha${note.noteData}', player));
