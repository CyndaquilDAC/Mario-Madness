...
class VlcBitmap extends Bitmap
{
  ...
  public function play(?source:String)
  {
    ...
    if (!inWindow)
    {
      if (source != null) libvlc.play(source);
      else
        libvlc.play();
    }
    else
    {
      if (source != null) libvlc.playInWindow(source);
      else
        libvlc.playInWindow();
  ...
  {
    #if (cpp && !mobile)
    if (libvlc != null && initComplete) return libvlc.getFPS();
    else
      return 0;
    #else
    return 0;
  ...
  {
    #if (cpp && !mobile)
    if (libvlc != null && initComplete) return libvlc.getTime();
    else
      return 0;
    #else
    return 0;
      ...
      if (isPlaying)
      {
        try
        {
          ...
          if (bufferMem != null)
          {
            ...
            if (libvlc.getPixelData() != null) bitmapData.setPixels(frameRect, Bytes.ofData(bufferMem));
  ...
  {
    #if (cpp && !mobile)
    if (libvlc != null && initComplete) return libvlc.getVolume();
    else
      return 0;
    #else
    return 0;
