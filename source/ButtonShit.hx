...
class ButtonGroup extends FlxGroup
{
  ...
  public function move(direction:Int):Void
  {
    ...
    if (cursors)
    {
      tweens.push(FlxTween.tween(cursorGroup.topleft, {x: temp.x - offset.x + temp.cursorOffsets[0] + 2, y: temp.y - offset.y + temp.cursorOffsets[1] + 3},
        0.33, {ease: FlxEase.cubeOut}));
      tweens.push(FlxTween.tween(cursorGroup.topright,
        {x: temp.width + temp.x - offset.x + temp.cursorOffsets[2] - 4, y: temp.y - offset.y + temp.cursorOffsets[3] + 3}, 0.33, {ease: FlxEase.cubeOut}));
      tweens.push(FlxTween.tween(cursorGroup.bottomright,
        {x: temp.width + temp.x - offset.x + temp.cursorOffsets[4] - 4,
          y: temp.height
          + temp.y
          - offset.y
      ...
      tweens.push(FlxTween.tween(cursorGroup.bottomleft,
        {x: temp.x - offset.x + temp.cursorOffsets[6] + 2, y: temp.height + temp.y - offset.y + temp.cursorOffsets[7] - 4}, 0.33, {ease: FlxEase.cubeOut}));
    }
  }
...
class Keyboard extends FlxGroup
{
  ...
  public function new(cam:FlxCamera, x:Int, y:Int, font:String, fontSize:Int, fontColor:FlxColor, charset:String, charsetCaps:String, charsetShift:String,
      offset:Bool, unpressed:FlxGraphicAsset, pressed:FlxGraphicAsset, mouseAway:FlxGraphicAsset, instant:Bool, cursor:FlxSprite, backing:FlxGraphicAsset,
      cursor2:FlxGraphicAsset, x2:Int, y2:Int, offsetX:Int, offsetY:Int, repeatDelay:Float, repeatPeriod:Float)
  {
    super();
    camera = cam;
