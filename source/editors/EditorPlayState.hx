...
class EditorPlayState extends MusicBeatState
{
            ...
            if (floorSus > 0)
            {
              for (susNote in 0...floorSus + 1)
              {
                ...
                var sustainNote:Note = new Note(daStrumTime
                  ...
                  + (Conductor.stepCrochet / FlxMath.roundDecimal(PlayState.SONG.speed, 2)), daNoteData,
                  oldNote, true);
      ...
      if (controlHoldArray.contains(true) || controlArray.contains(true))
      {
        ...
        if (controlArray.contains(true))
        {
          for (i in 0...controlArray.length)
          {
            ...
            else if (canMiss && controlArray[i]) noteMiss(i, true);
          }
        }
      }
    }
  ...
  private function popUpScore(note:Note = null):Void
  {
    ...
    for (i in seperatedScore)
    {
      ...
      if (combo >= 10 || combo == 0) comboGroup.add(numScore);
