...
class NotesSubstate extends MusicBeatSubstate
{
  ...
  function changeSelection(change:Int = 0)
  {
    ...
    for (i in 0...grpNumbers.length)
    {
      ...
      item.alpha = 0.4;
      item.color = 0xFF2E1010;
      if ((curSelected * 3) + typeSelected == i)
      {
        item.alpha = 1;
        item.color = 0xFF982B2B;
      }
    }
    for (i in 0...grpNotes.length)
    {
  ...
  override function update(elapsed:Float)
  {
    ...
    else
    {
      if (FlxG.keys.pressed.LEFT || FlxG.keys.pressed.RIGHT)
      {
        ...
        if (holdTime > 0.5 || FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.RIGHT) switch (options[curSelected])
        {
          case 'Framerate':
...
class MarioSubstate extends MusicBeatSubstate
{
  ...
  static var bglist:Array<String> = ['Mushroom Kingdom',];

  final unlockBG:Array<Bool> = [
        ...
        if (FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.RIGHT)
        {
          ...
          switch (options[curSelected])
          {
            case 'Menu BG':
              ...
              if (ClientPrefs.menuBG < 0) ClientPrefs.menuBG = bglist.length - 1;
              else if (ClientPrefs.menuBG > bglist.length - 1) ClientPrefs.menuBG = 0;
          }
          reloadValues();
        }
      }
