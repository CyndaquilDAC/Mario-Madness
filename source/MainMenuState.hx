...
class MainMenuState extends MusicBeatState
{
  ...
  override function create()
  {
    ...
    for (info in menuInfo)
    {
      ...
      if (pos.x == -1) info.group.screenCenter(X);
      else
        info.group.x = pos.x;

      if (pos.y == -1) info.group.screenCenter(Y);
      else
        info.group.y = pos.y;
    ...
    if (showMsg == 0) (new FlxTimer()).start(0.6, function(t:FlxTimer) {
  ...
  override function update(elapsed:Float)
  {
    ...
    if (FlxG.sound.music.volume < 0.8 && !selectedSomethin) FlxG.sound.music.volume += 0.5 * FlxG.elapsed;
    ...
    if (!selectedSomethin && !smOpen)
    {
      ...
      if (lastPressed != -1 && canUseKeyCombos) for (fullPhrase => func in keyCombos)
      {
  ...
  function goToState()
  {
    ...
    for (group in menuGroups)
    {
      group.forEachAlive((_) -> {
        if (findGroupLevel(group) != currentLevel || _.ID != curSelected) FlxTween.tween(_, {alpha: 0}, 0.3,
          {
            ease: FlxEase.circInOut
          });
      });
    ...
    if (ClientPrefs.flashing) FlxFlicker.flicker(menuInfo[currentLevel].group.members[curSelected], 1, 0.06, false, false);

    if (!ClientPrefs.flashing
      || menuInfo[currentLevel].choices[curSelected] == "MainGame") FlxTween.tween(menuInfo[currentLevel].group.members[curSelected], {alpha: 0}, .4);

    if (menuInfo[currentLevel].choices[curSelected] == "MainGame") for (spr in [fondo11, estatica])
    {
  ...
  private function findGroupLevel(grp:FlxTypedSpriteGroup<FlxSprite>):Int
  {
    for (info in menuInfo)
    {
      if (info.group == grp) return menuInfo.indexOf(info);
    }
    return 0;
  }
  ...
  function changeItem(?huh:Int = 0, ?arrowCheck:Bool = true)
  {
    ...
    if (arrowCheck)
    {
      if (curSelected >= menuInfo[currentLevel].group.length) curSelected = 0;
      if (curSelected < 0) curSelected = menuInfo[currentLevel].group.length - 1;
    }
  ...
  function keyInput(k:FlxKey):String
  {
    ...
    switch (asString)
    {
      ...
      case 'backslash':
        return '\\';
      case 'any' | 'none' | 'printscreen' | 'pageup' | 'pagedown' | 'home' | 'end' | 'insert' | 'escape' | 'delete' | 'backspace' | 'capslock' | 'enter' |
        'shift' | 'control' | 'alt' | 'f1' | 'f2' | 'f3' | 'f4' | 'f5' | 'f6' | 'f7' | 'f8' | 'f9' | 'f0' | 'tab' | 'up' | 'down' | 'left' | 'right':
        return '';
      case 'space':
        return ' ';
      case 'slash':
  ...
  function changeLevel(?duh:Int = 0, ?arrowCheck:Bool = true)
  {
    ...
    if (arrowCheck)
    {
      if (currentLevel >= menuInfo.length) currentLevel = 0;
      if (currentLevel < 0) currentLevel = menuInfo.length - 1;
  ...
  {
    if (space == null) space = FlxPoint.get(12.5, 12.5);
        ...
        new FlxTimer().start(2, function(tmr:FlxTimer) {
          ...
          new FlxTimer().start(1, function(tmr:FlxTimer) {
            ...
            if (!foundFile)
            {
              ...
              if (#if sys FileSystem.exists(fileName) #else OpenFlAssets.exists(fileName) #end) foundFile = true;
