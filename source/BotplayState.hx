...
class BotplayState extends MusicBeatState
{
  ...
  override function create()
  {
    ...
    msgText = new FlxText(175, 169 - 720, 1180,
    ...
    FlxTween.tween(msgText, {y: 169}, 0.4,
      {
    ...
    if (canSelect)
    {
      buttons.forEach(function(spr:FlxSprite) {
        ...
        if (FlxG.mouse.justPressed && FlxG.mouse.overlaps(spr))
        {
          // goToState();
          canSelect = false;
          FlxTween.tween(spr.scale, {x: 0.9, y: 0.9}, 0.05,
            {
              ease: FlxEase.cubeIn,
              onComplete: function(twn:FlxTween) {
                spr.animation.play('select');
                edges.visible = false;
                FlxTween.tween(spr.scale, {x: 1, y: 1}, 0.25,
                  {
  ...
  function select(selected:Int)
  {
    if (selected != curSelect)
    {
      ...
      edges.forEach(function(edge:FlxSprite) {
        ...
        edgeTweens.push(FlxTween.tween(edge, {x: edge.x + (4 * edgepos[i][2]), y: edge.y + (4 * edgepos[i][3])}, 0.4,
  ...
  function selected(selected:Int)
  {
    ...
    FlxTween.tween(msgText, {y: msgText.y + 720}, 0.4,
      {
        ease: FlxEase.cubeIn,
        onComplete: function(twn:FlxTween) {
          switch (selected)
